...
def parse_uspto_file(bs, keep_log: bool = False):
    ...
    for related_doc_bs in (
        related_docs_bs.find_all(recursive=False) if related_docs_bs else []
    ):
        related_doc = {
        ...
        if related_doc_bs.name in [
            ...
            for documents_bs in related_doc_bs.find_all(
                re.compile("(parent|child)-doc(ument)?$")
            ):
                for doc_bs in documents_bs.find_all("document-id"):
                    if doc_bs.parent.name == "parent-grant-document":
                        ...
                        related_doc["metadata"]["parent_pct_number"] = doc_bs.find(
                            "doc-number"
                        ).text
                        related_doc["metadata"]["parent_pct_country"] = doc_bs.find(
                            "country"
                        ).text
                        related_doc["metadata"]["parent_pct_date"] = getattr(
                            doc_bs.find("date"), "text", None
                        )
                    elif doc_bs.parent.name == "parent-doc":
                        related_doc["country"] = doc_bs.find("country").text
                        related_doc["metadata"]["application_number"] = doc_bs.find(
                            "doc-number"
                        ).text
                        related_doc["metadata"]["application_date"] = getattr(
                            doc_bs.find("date"), "text", None
                        )
                    elif doc_bs.parent.name == "child-doc":
                        related_doc["metadata"][
                            "child_application_number"
                        ] = doc_bs.find("doc-number").text
                        related_doc["metadata"]["parent_country"] = doc_bs.find(
                            "country"
                        ).text
            ...
            related_doc["metadata"]["application_date"] = related_doc_bs.find(
                "date"
            ).text
            ...
            raise KeyError(
                f"'{related_doc_bs.name}' is not setup to be included in referential documents."
            )
...
def load_local_files(
    ...
    if not (isinstance(push_to, str) and push_to.endswith(".jsonl")) and not isinstance(
    ...
    for filename in filenames:
        ...
        for patent in xml_splits:
            ...
            try:
                uspto_patent = parse_uspto_file(bs=application, keep_log=keep_log)
                if isinstance(push_to, str) and push_to.endswith(".jsonl"):
